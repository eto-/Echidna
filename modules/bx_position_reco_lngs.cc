...
// ctor
bx_position_reco_lngs::bx_position_reco_lngs() : bx_base_module("bx_position_reco_lngs", bx_base_module::main_loop) {
  require_event_stage (bx_detector::laben, bx_base_event::clustered);
  current_module = this; 	// see minuit root workaround
}

void bx_position_reco_lngs::begin () {
//
...
//  get_message(bx_message::log) << "T0 starting point" << f4_t_0 << dispatch;
  ...
  for ( int i=0; i<(int)pdf_time_v.size(); i++ ) {
    ...
    pdf_data2[i] = pdf_02[i];
    pdf_data3[i] = pdf_03[i];
    pdf_data4[i] = pdf_04[i];
    pdf_data5[i] = pdf_05[i];
    pdf_data6[i] = pdf_06[i];
    pdf_data7[i] = pdf_07[i];
    pdf_data8[i] = pdf_08[i];
    pdf_data9[i] = pdf_09[i];
    pdf_data10[i] = pdf_10[i];
  }
...
bx_echidna_event* bx_position_reco_lngs::doit (bx_echidna_event *ev) {
...
//
  ...
  ev->get_laben ().mark_stage (bx_base_event::reconstructed_lngs);
  
  return ev;
}

void bx_position_reco_lngs::end () {
...
//       get_message(bx_message::warn) <<" costheta < -1 !!! in event number =    "<< n_evento << "    costheta =   "<< costheta << dispatch;
    ...
    if(pdf_value <= 0.){
     get_message(bx_message::warn) << "bx_position_reco_lngs: in MY_FCN pdf_value < 0!!! " << dispatch;
    }
  }
