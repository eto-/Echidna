...
if [ $? -ne 0 ]; then
  echo "pbs_echidna works only in subforlders of home directory ($HOME), while you are in $PWD"
  exit 1
fi
...
TMPFILE=`mktemp /tmp/echidna.XXXXXX`
if [ $? -ne 0 ]; then
...
# Launch the job
qsub -N $jobname -d $PWD $queue $interactive $mail $streams $TMPFILE
