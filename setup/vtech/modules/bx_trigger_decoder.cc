 ...
 *
 * $Id: bx_trigger_decoder.cc,v 1.1 2012/11/25 22:15:51 mosteiro Exp $
 *
...
bx_echidna_event* bx_trigger_decoder::doit (bx_echidna_event *ev) {
  ...
  if ( (ev-> get_event_number () != 0) && (dt_gps_ppc0 > 200 || dt_gps_ppc0 < -200 )){
    get_message(bx_message::error) << " for event= " << ev->get_trigger ().get_evid () << " wrong gps time, fixing did not work " << er.get_time_t() << " " << ev->get_trigger().get_trg_time() << dispatch;    
  }
...
/*  // compute number of seconds since 01-01-2000  */
  ...
  if (timet > 1230768000L) sec2k += 2;  //31 Dic 2008 2 leap seconds
 ...
 * Revision 1.1  2012/11/25 22:15:51  mosteiro
 * files for vtech setup
 *
