...
# PWD CHECK
echo $PWD | grep -q $HOME
if [ $? -ne 0 ]; then
  echo "pbs_echidna works only in subforlders of ($HOME), while you are in $PWD"
  exit 1
fi
...
#!/bin/sh
#set -x
.  /home/pablo/root-5.20/bin/thisroot.sh 
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$PWD:/usr/lib/:/e/h.0/home/local/fftw3/lib
hostname
...
qsub -l walltime=18:00:00 -N $jobname -d $PWD $queue $interactive $mail $streams $TMPFILE
