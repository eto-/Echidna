...
const char LMSDialog::logic[][1024] = {"OR", "AND", "AND NOT"};

const char LMSDialog::fields[][1024] =
{ "", "Multiplicity", "ChargeStatus", "ChargeBaseStatus", "ChargePeakStatus", "TimingStatus"};

const char LMSDialog::types1[][1024] =
{ "",
...
LMSDialog::LMSDialog(QWidget *parent)
  : QDialog(parent), field1(0), field2(0), field3(0), type1(0), type2(0), type3(0)
{
  ...
  setWindowTitle(tr("Laben Channels status"));
}
...
void LMSDialog::enableProceedButton()
{
...
void LMSDialog::setField1(int f_id){
...
void LMSDialog::setField2(int f_id){
...
void LMSDialog::setField3(int f_id){
...
void LMSDialog::setType1(const QString & text){
  if (type1){ delete type1; type1 = 0; }
  if (!text.isEmpty()) type1 = new QString(text);
  enableProceedButton();
}

void LMSDialog::setType2(const QString & text){
  if (type2){ delete type2; type2 = 0; }
  if (!text.isEmpty()) type2 = new QString(text);
  enableProceedButton();
}

void LMSDialog::setType3(const QString & text){
  if (type3){ delete type3; type3 = 0; }
  if (!text.isEmpty()) type3 = new QString(text);
  enableProceedButton();
}

void LMSDialog::showChannels()
{
  int t_used[3] = {1, 1, 1};
  ...
  sprintf(the_query, "SELECT \"ChannelID\", \"%s\", \"%s\", \"%s\" FROM \"LabenChannelsProperties\" \
  ...
  mainWindow::get()->dispatchMsg(str->sprintf(" >>> Laben Channels Status: run %d\n status:", run), 0);
  ...
  for (int i = 0; i < lines; ++i){
    ...
    if (selection_found){
      ++total_found;
      mainWindow::get()->dispatchMsg(str->sprintf(" %4d % 5.1f % 5.1f % 5.1f % 5.1f\n", i+1, peak_values[i], sig_values[i], mean_values[i], rms_values[i]), 0);
    }
  ...
  if (prnCheckBox->isChecked()){
    total_found = 0;
    mainWindow::get()->dispatchMsg(str->sprintf (" disconnected channels: "), 0);
    for (int i = 0; i < CH_NUM; ++i){
      if (dsc_ch[i]){
	mainWindow::get()->dispatchMsg(str->sprintf ("%d ", i+1), 0);
	++total_found;
      }
    }
    mainWindow::get()->dispatchMsg(str->sprintf("\n total: %d\n", total_found), 0);
